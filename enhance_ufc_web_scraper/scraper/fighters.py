        ...
        with open(file_path + '/' + 'ufc_fighter_data.csv', 'a+') as ufc_fighters:
            ...
            for url in fighter_urls:
                try:
                    ...
                    mdict = {}
                    for i in range(5,len(details)):
                        try:
                            topic = str(details[i]).split(":")[0].split("\n")[-1].strip()
                            j = [ i for i in str(details[i]).split(":")[1].split("\n") if len(re.findall(r'\d+',i)) > 0 ]
                            if len(j) > 0 :
                                j = j[0].strip()
                            else:
                                j = None
                            mdict["fighter_" + topic] = j
                        except:
                            pass
                    ...
                    writer.writerow([fighter_f_name.strip(), 
                                     ...
                                     fighter_dob[0:10],
                                     mdict['fighter_SLpM'],
                                     mdict['fighter_Str. Acc.'],
                                     mdict['fighter_SApM'],
                                     mdict['fighter_Str. Def'],
                                     mdict['fighter_TD Avg.'],
                                     mdict['fighter_TD Acc.'],
                                     mdict['fighter_TD Def.'],
                                     mdict['fighter_Sub. Avg.'],
                                     fighter_w,
                                     fighter_l,
                                     fighter_d,
                                     fighter_nc_dq,
